...
def get_args_parser(cfg):
    ...
    if os.name == 'nt':
        ...
        model_path = project_folder_path + "\\models\\BERT-BiLSTM-CRF"
        #model_path = project_folder_path + "\\inputData\\formal_training\\output"
    else:
        project_folder_path = "/".join(os.path.abspath(__file__).split("/")[: -4])
        bert_path = project_folder_path + "/models/bert/chinese_L-12_H-768_A-12"
        model_path = project_folder_path + "/models/BERT-BiLSTM-CRF"
    ...
    group1.add_argument('-model_dir', type=str, default=model_path,
                        help='directory of a pretrained BERT model')
    group1.add_argument('-model_pb_dir', type=str, default=param_dict["model_pb_dir"],
                        help='directory of a pretrained BERT model')
    ...
    group2.add_argument('-max_seq_len', type=int, default=param_dict["max_seq_len"],
    ...
    group3.add_argument('-port', '-port_in', '-port_data', type=int, default=param_dict["port"],
                        help='server port for receiving data from client')
    group3.add_argument('-port_out', '-port_result', type=int, default=param_dict["port_out"],
                        help='server port for sending result to client')
    group3.add_argument('-http_port', type=int, default=param_dict["http_port"],
                        help='server port for receiving HTTP requests')
    group3.add_argument('-http_max_connect', type=int, default=param_dict["http_max_connect"],
                        help='maximum number of concurrent HTTP connections')
    group3.add_argument('-cors', type=str, default=param_dict["cors"],
                        help='setting "Access-Control-Allow-Origin" for HTTP requests')
    group3.add_argument('-num_worker', type=int, default=param_dict["num_worker"],
                        help='number of server instances')
    group3.add_argument('-max_batch_size', type=int, default=param_dict["max_batch_size"],
                        help='maximum number of sequences handled by each worker')
    group3.add_argument('-priority_batch_size', type=int, default=param_dict["priority_batch_size"],
                        help='batch smaller than this size will be labeled as high priority,'
                             'and jumps forward in the job queue')
    group3.add_argument('-cpu', action='store_true', default=param_dict["cpu"],
    ...
    group3.add_argument('-gpu_memory_fraction', type=float, default=param_dict["gpu_memory_fraction"],
                        help='determine the fraction of the overall amount of memory \
                        that each visible GPU should be allocated per worker. \
                        Should be in range [0.0, 1.0]')
    group3.add_argument('-device_map', type=int, nargs='+', default=param_dict["device_map"],
    ...
    group2.add_argument('-label_list', type=str, default=param_dict["label_list"],
                        help='the label list you used when training')
...
def get_run_args(cfg, parser_fn=get_args_parser, printed=True):
    args = parser_fn(cfg = cfg).parse_args()
    if printed:
